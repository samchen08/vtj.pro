...
export const vWheelinput: Directive<HTMLElement> =  {
  mounted(el, binding) {
    const { value } = binding;
    nextTick(() => {
      if (Object.prototype.toString.call(value) === '[object Object]') {
        ...
        // 获取限制值
        const getLimitValue = (value: string | number): number | 'infinity' => {
        ...
        for (let index = 0; index < inputs.length; index++) {
          const input = inputs[index];input.addEventListener('wheel', (e: { preventDefault: () => void; deltaY: number; }) => {
            e.preventDefault();
            const now = Date.now();
