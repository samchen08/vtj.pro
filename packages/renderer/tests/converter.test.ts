...
test('rpxToPx converts rpx to px correctly', () => {
  // 测试基础转换
  expect(rpxToPx(window, 750)).toBe(750); // 750rpx * 750 / 750 = 750px
  expect(rpxToPx(window, 375)).toBe(375); // 375rpx * 750 / 750 = 375px
  expect(rpxToPx(window, 100)).toBe(100); // 100rpx * 750 / 750 = 100px

  // 测试自定义基准宽度
  expect(rpxToPx(window, 750, 375)).toBe(1500); // 750rpx * 750 / 375 = 1500px
});
...
test('rpxToPx handles non-browser environment', () => {
  ...
  expect(rpxToPx(undefined, 750)).toBe(375);
...
test('convertCssRpx converts simple rpx values', () => {
  ...
  const result = convertCssRpx(window, css);
...
// Test for single quote coverage (line 50)
test('convertCssRpx does not convert rpx inside single-quoted strings', () => {
  const css = `.container { content: '10rpx'; font-family: 'test-rpx'; }`;
  const result = convertCssRpx(window, css);
  expect(result).toBe(css);
});
import { expect, test, vi, beforeEach } from 'vitest';
