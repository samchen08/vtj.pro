        ...
        <ElRow :gutter="20">
          <ElCol :span="16">
            <ElCarousel indicator-position="outside" height="300px">
              <ElCarouselItem v-for="(item, index) in state.carouselItems" :key="index">
            ...
            <ElCard
              v-for="article in state.latestArticles"
              :key="article.id"
              class="article-card"
            >
              <template #header>
              ...
              <div class="article-meta">
                ...
                <ElButton type="text" @click="viewArticle(article.id)">阅读更多</ElButton>
              </div>
            </ElCard>
            <ElPagination
              background
...
import { Clock, User, Search } from '@vtj/icons';
import {
  ElContainer,
  ElHeader,
  ElMain,
...
export default defineComponent({
  ...
  emits: [],
  setup(props) {
    const provider = useProvider({ id: '17x15kwp', version: '' });
    const state = reactive({
      ...
      latestArticles: [],
      hotTags: ['Vue', 'JavaScript', 'CSS', 'Element-Plus', '数据可视化', 'UI/UX'],
      totalArticles: 0,
      pageSize: 5,
      currentPage: 1
    });

    return { state, props, provider, Clock, User, Search };
  },
  methods: {
    ...
    viewArticle(id) {
      console.log('查看文章:', id);
      // 可以在这里使用 this.$router.push('/article/' + id);
    },
