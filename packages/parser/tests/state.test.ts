...
test('should fix increment operator on state properties', () => {
    const source = `
<template>
  <div>
...
`;
  ...
  test('should fix complex expressions with state properties', () => {
    const source = `
<template>
  <div>
    <p>{{ showFlag ? count + 1 : 0 }}</p>
    <p v-if="showFlag && count > 0">Visible</p>
  </div>
</template>
...
`;
    ...
    expect(result.fixed).toBe(true);
    expect(result.fixedCode).toContain('state.count');
    expect(result.fixedCode).toContain('state.showFlag');
  });
import { expect, test, describe } from 'vitest';
import { checkAndFixStatePrefix } from '../src/tools/state';
