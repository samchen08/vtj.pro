...
> **⚠️ 平台限制：** `@vtj/icons` 仅适用于 **Web** 和 **H5** 平台，**不可用于 uni-app**。
>
> - **Web 端：** 可使用 `XIcon` 组件或直接导入图标组件
> - **H5 端：** 禁止使用 `XIcon`，需直接按需导入图标组件，以原生标签名渲染
> - **uni-app：** 不支持 `@vtj/icons`，请使用平台原生图标方案
>
> 详见 [7.5 H5 平台图标用法](#75-h5-平台图标用法)。
...
> **`@vtj/icons` 仅适用于 Web 和 H5 平台，不可用于 uni-app。** H5 平台禁止使用 `XIcon`（`XIcon` 是 `@vtj/ui` 的 PC 端组件），需直接按需导入图标组件，以原生 Vue 组件方式使用。

#### Iconfont 图标（推荐）

Iconfont 图标是最适合 H5 平台的图标类型，导入后直接使用标签名，通过 CSS 控制样式：

```vue
<template>
  <div class="toolbar">
    <VtjIconSetting class="icon" @click="onSetting" />
    <VtjIconSearch class="icon" @click="onSearch" />
    <VtjIconUser class="icon" />
    <VtjIconArrowRight class="icon" />
  </div>
</template>
...
SVG 组件图标导入后直接使用标签名，支持 `colors` 属性按 path 着色：

```vue
<template>
  <div class="toolbar">
    <Fullscreen :size="24" />
    <UserLogin :size="24" :colors="['#333', '#409EFF']" />
  </div>
</template>
...
Element Plus 图标在 H5 平台可直接使用标签名，或通过 `<el-icon>` 包裹控制尺寸：
...
<script setup>
  import { Edit, Delete, Search } from '@vtj/icons';
</script>
```

#### Assets 图标

Assets 图标本质是 SVG 路径，H5 平台通过 `<img>` 标签的 `src` 属性渲染：

```vue
<template>
  <div class="assets-demo">
    <img :src="IconButton" style="width: 40px; height: 40px;" />
    <img :src="IconTable" style="width: 40px; height: 40px;" />
  </div>
</template>
...
<template>
  <div class="page">
    <!-- 顶部导航栏 -->
    <div class="navbar">
      <VtjIconBack class="nav-icon" @click="onBack" />
      <span class="title">{{ __i18n.t('page.title') }}</span>
      <VtjIconMore class="nav-icon" @click="onMore" />
    </div>

    <!-- 搜索栏 -->
    <div class="search-bar">
      <VtjIconSearch class="search-icon" />
      <input v-model="keyword" :placeholder="__i18n.t('common.search')" />
    </div>

    <!-- 操作按钮 -->
    <div class="actions">
      <div class="action-item" @click="onEdit">
        <VtjIconEdit class="action-icon" />
        <span>{{ __i18n.t('common.edit') }}</span>
      </div>
      <div class="action-item" @click="onDelete">
        <VtjIconDelete class="action-icon" color="#F56C6C" />
        <span>{{ __i18n.t('common.delete') }}</span>
      </div>
      <div class="action-item" @click="onRefresh">
        <VtjIconRefresh class="action-icon" />
        <span>{{ __i18n.t('common.refresh') }}</span>
      </div>
    </div>
  </div>
</template>
...
> **H5 平台图标使用要点：**
>
> - **禁止使用 `XIcon`**，直接导入图标组件，以原生标签名使用
> - `@vtj/icons` 不可用于 uni-app 平台
> - Iconfont 图标通过 CSS `font-size` 和 `color` 控制大小和颜色
> - Element Plus 图标可直接使用标签名或通过 `<el-icon>` 包裹
> - Assets 图标在 H5 平台通过 `<img :src="IconXxx" />` 渲染
...
2. **按需导入需配合 XIcon（仅 Web 平台）：** 未全局注册时，Web 平台需从 `@vtj/ui` 导入 `XIcon`，通过 `:icon="IconName"` 传递图标组件引用；H5 平台禁止使用 `XIcon`，应直接使用导入的组件标签名
3. **`icon` 属性传递组件引用：** `XIcon` 的 `:icon` 接收 Vue 组件对象（即 `import` 后的变量名），并非字符串路径
4. **Assets 图标用 `src` 属性：** Assets 图标（`IconXxx`）Web 平台通过 `XIcon` 的 `:src="IconName"` 属性渲染，H5 平台通过 `<img :src="IconXxx" />` 渲染
5. **XIcon 支持 `size` 和 `color`：** `:size` 可传数字或 `'small'`/`'default'`/`'large'`/`'inherit'`，`color` 传 CSS 色值字符串
6. **Iconfont 图标默认颜色：** `VtjIconXxx` 通过 `XIcon` 渲染时，`color` 属性控制颜色；全局注册或 H5 平台直接使用时通过 CSS `color` 控制
7. **SVG 组件 `colors` 属性：** SVG 组件支持 `colors` 数组按 path 顺序着色，但需通过 `<component :is="IconName">` 或全局注册后直接标签使用
8. **无需耗时搜索图标名称：** 上述列表已覆盖所有图标，直接从列表中选取即可
9. **平台兼容性：** `@vtj/icons` 仅适用 Web / H5 平台，不可用于 uni-app。H5 平台需直接按需导入图标组件并以原生标签名使用（禁用 `XIcon`），详见 [7.5 H5 平台图标用法](#75-h5-平台图标用法)
